...
struct polynomial subtract_polynomials(struct polynomial P, struct polynomial Q)
{
	struct polynomial difference;
	difference.degree = MAX(P.degree, Q.degree);
	if (P.degree == Q.degree) { /* Adjust degree if leading terms cancel out */
		while (P.coeffs[difference.degree] == Q.coeffs[difference.degree] && difference.degree > 0) {
			difference.degree--;
		}
	}
	difference.coeffs = (int*)safe_malloc(MAX_POLYNOMIAL_SIZE * sizeof(int));

	/*Each of the coefficients of the sum is set to the sum of the corresponding coefficients from P and Q*/
	for (int index = 0; index <= difference.degree; index++) {
		difference.coeffs[index] = 0;
		if (index <= P.degree)
			difference.coeffs[index] += P.coeffs[index];
		if (index <= Q.degree)
			difference.coeffs[index] -= Q.coeffs[index];
	}
