...
/*Function which sets corrects the degree of a polynomial by ignoring leading zeroes*/
void adjust_degree(struct double_polynomial* P) {
	for (int index = P->degree; index >= 0; index--) 
		if (P->coeffs[index] != 0) {
			P->degree = index;
			return;
		}
